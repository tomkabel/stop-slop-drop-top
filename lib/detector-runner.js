...
const { analyzeText, integrateScores, saveToHistory, getVerdict } = require('./detection-core');
const { visualOutput, compactOutput, minimalOutput } = require('./visual-output');
...
async function runDetection(text, detectorNames = [], options = {}) {
  const { verbose = true, cache: useCache = true, ttl, threshold = 50, saveHistory = true } = options;
  ...
  if (useHeuristic) {
    ...
    results.heuristic = analyzeText(text);
  ...
  const finalResult = {
    ...
    detectors: results,
    integrated,
    summary: generateSummary(results, integrated, threshold),
  ...
  if (saveHistory) {
    saveToHistory({
      ...
      textLength: text.length,
      aiPercent: integrated.integratedPercent ?? results.heuristic?.results?.aiPercentage,
 ...
 */
function generateSummary(results, integrated, threshold = 50) {
  let aiCount = 0;
  let humanCount = 0;
  const scores = [];
  let explanation = [];
  ...
  for (const [name, result] of Object.entries(results)) {
    ...
    if (aiPercent !== undefined && aiPercent !== null) {
      ...
      if (aiPercent >= threshold) aiCount++;
      else humanCount++;
    }
    
    if (name === 'heuristic' && result.explanation) {
...
function quickCheck(text, options = {}) {
  ...
  const verdict = getVerdict(result.results.aiPercentage);
  return {
    ...result,
    isHighAI: result.results.aiPercentage >= threshold,
    threshold,
    verdict
  };
}
